...
export async function errorMessage(
  ...
  if ((message as {resp: Response}).resp instanceof window.Response) {
    ...
    message = (message as {message: string}).message // for handling Errors {name:String, message:String}
  ...
  document.dispatchEvent(new CustomEvent<ErrorSkEventDetail>('error-sk', { detail: detail, bubbles: true }));
}
